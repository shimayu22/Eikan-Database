...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if previous_game.exists():
            previous_game_pitchers = PitcherResults.objects.select_related('game_id').filter(
                game_id=previous_game.latest('competiton_round')).order_by('player_id')
            for pts in ctx['pitcher_total_results']:
                for pgp in previous_game_pitchers:
                    if pts.player_id == pgp.player_id:
                        pts.games_started = pgp.games_started
